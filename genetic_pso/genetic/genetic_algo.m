function [y,ev,nfe,time,gbest_value_hist] = genetic_algo(dim,max_gen,ps,pm,pc,max_var,evaluation_function,opt_type)
 tic; %tic start a timer to measure the period of G-algorithm
%initialize population
x = (max_var(2)-max_var(1))*rand([ps dim]) + max_var(1);
%iterate for max_gen
nfe = double(0);
gbest_value_hist = nan(max_gen-1,1);
for gen=2:max_gen
