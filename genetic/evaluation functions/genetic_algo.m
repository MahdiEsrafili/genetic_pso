...
for gen=2:max_gen
    ...
    temp = size(pop_index);
    child_array = zeros(temp(2),dim);
    %cross over one by one. I cross over 2n+1 and 2*n+2
    for i=0:uint8(cocn/2)-1
        ...
        child_array(2*i+1,1:co_point(1)) = x(2*i+1,1:co_point(1)); % put the first part of parents into children
        child_array(2*i+2,1:co_point(1)) = x(2*i+2,1:co_point(1));
        child_array(2*i+1,co_point(1)+1:co_point(2)-1) = x(2*i+2,co_point(1)+1:co_point(2)-1); %put the middle part of parents into children 
        child_array(2*i+2,co_point(1)+1:co_point(2)-1) = x(2*i+1,co_point(1)+1:co_point(2)-1);
        child_array(2*i+1,co_point(2):end) = x(2*i+1,co_point(2):end); % put the last part of parents into last part of children
        child_array(2*i+2,co_point(2):end) = x(2*i+2,co_point(2):end);
    end
    %mutation
    mutn = uint8(ps*pm); % mutn = mutation number
    mut_index = randperm(ps,uint8(mutn)); % index of parent selected for mutation
    for i = 1:mutn
    ...
    temp = size(full_pop);
    evaluated_value = zeros(temp(1),1); % initiate array to increase the speed of calculation
    for j=1:temp(1)
